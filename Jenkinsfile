pipeline {
    ...
    environment {
        DOCKER_IMAGE_NAME = 'medgm/real-estate-notification-service'
    ...
    stages {
        ...
        stage('Verify Project Layout') {
            steps {
                script {
                    sh '''
                        echo "Verifying notification-service repository layout..."
                        pwd
                        ls -la
                        if [ ! -f pom.xml ]; then
        ...
        stage('Build & Test') {
            steps {
                script {
                    sh '''
                        echo "Building Spring Boot Notification Service..."
                        chmod +x ./mvnw
            ...
            steps {
                script {
                    sh """
                        ...
                        docker tag ${DOCKER_IMAGE_NAME}:${BUILD_NUMBER} localhost:5000/real-estate-notification-service:${BUILD_NUMBER}
                        docker tag ${DOCKER_IMAGE_NAME}:${BUILD_NUMBER} localhost:5000/real-estate-notification-service:latest

                        docker push localhost:5000/real-estate-notification-service:${BUILD_NUMBER}
                        docker push localhost:5000/real-estate-notification-service:latest

                        echo "Images in local registry:"
                        docker images | grep real-estate-notification-service
                    """
                }
            }
        }
